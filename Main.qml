...
Window {
        ...
        RowLayout {
         ...
         ColumnLayout {
                ...
                Row {
                    ...
                    Repeater {
                        model: Backend.audioLevels 
                        delegate: Rectangle {
                            width: (visualizerRow.width / 32) - 2 
                            height: Math.max(2, (modelData !== undefined ? modelData : 0) * visualizerRow.height)
                ...
                RowLayout {
                    ...
                    Slider {
                        ...
                        value: (pressed || Backend.duration <= 0) ? value : Backend.position
    ...
    FileDialog { 
        id: fileDialog
        nameFilters: ["Música (*.mp3 *.m4a *.mp4 *.flac)"]
        fileMode: FileDialog.OpenFiles
        onAccepted: { for(var i=0; i<selectedFiles.length; i++) Backend.addToCurrentPlaylist(selectedFiles[i]) } 
    }
    ...
    Dialog { 
        ...
        ColumnLayout { 
            TextField { id: playlistNameField; placeholderText: "Nombre..."; color: "black" } 
        } 
        onAccepted: { Backend.createPlaylist(playlistNameField.text); Backend.switchPlaylist(playlistNameField.text); playlistNameField.text = "" } 
    }
